...
if (isset($_POST['sendMessage'])) {
    ...
    if (empty($errors)) {
        ...
        $query = "INSERT INTO messages(senderId,senderType,recieverId,recieverType,message,isRead,senderName,timeStamp) VALUES('{$adminid}','admin',99999,'user','{$message}',0,'{$name}','{$timeStamp}')";
        $result = mysqli_query($connection, $query);
        verifyQuery($result);
